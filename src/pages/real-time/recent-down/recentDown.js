...
let notPresent = true;
let SouthRecent = async (complete_date,layer,view) => {
      ...
      await southCPELayer.queryFeatures(south).then( async function (response) {
              ...
              if (c_date.getHours() === d.getHours()) {
                ...
                if (Math.abs(diff) <= 70) {
                  loadModules(["esri/Graphic"], { css: false }).then(
                    ...
                    //  console.log(recentDownSouth.graphics)
                     await recentDownSouth.graphics.add(pointGraphic);
                    }
                  );
                }
              }
