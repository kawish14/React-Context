...
export default class NorthPOP extends React.Component {
  ...
  componentDidMount() {
    ...
    let northCPELayer = view.northCPELayer;
    ...
    northPOP.renderer = POPrender;
    northPOP.labelingInfo = labelClassZone;
    ...
    let relatedTable = {
          ...
          _this.props.view.hitTest(event).then(function (response) {
            ...
            if (response.results.length) {
              ...
              response.results.map((e) => {
                if (e.graphic.layer) {
                    if (e.graphic.layer.title === "North POP") {
                        
                        northPOP.popupTemplate.actions = [relatedTable];
      ...
      _this.props.view.popup.on("trigger-action", function (event) {
        if (event.action.id === "related-tableCPENorth") {
          /*   const fileType =
          ...
          loadModules(["esri/popup/content/CustomContent"], {
            ...
            northPOP.popupTemplate = popupTemplate
