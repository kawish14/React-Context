...
export default class CentralPOP extends React.Component {
  ...
  componentDidMount() {
    ...
    let centralPOP = context.centralPOP;
    let centralCPELayer = context.centralCPELayer;
    ...
    const queryParams = centralCPELayer.createQuery();
        ...
        _this.props.view.hitTest(event).then(function (response) {
          ...
          if (response.results.length) {
            ...
            response.results.map((e) => {
              if (e.graphic.layer) {
                if (e.graphic.layer.title === "Central POP") {
                  centralPOP.popupTemplate.actions = [relatedTable];
            ...
            centralCPELayer.queryFeatures(queryParams).then(function (results) {
    ...
    _this.props.view.popup.on("trigger-action", function (event) {
      if (event.action.id === "related-tableCPECentral") {
        /*   const fileType =
        ...
        loadModules(["esri/popup/content/CustomContent"], {
          ...
          centralPOP.popupTemplate = popupTemplate;
        });
      }
    });
    ...
    return (
      ...
      >
        ...
        <tbody>
          <tr>
            <td className="data">
              {this.state.cards === -Infinity ? 0 : this.state.cards}
            </td>
            <td className="data">{this.state.used_ports}</td>
            <td className="data">
              {this.state.free_ports === -Infinity ? 0 : this.state.free_ports}
            </td>
